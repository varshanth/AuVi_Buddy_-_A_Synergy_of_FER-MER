...
def fer_dense_net(_input_shape, _n_classes):
	...
	_n_dense_layers_per_block = (6, 12, 24, 16) # Number of Dense Layers per block
	...
	intermed = Conv2D(_d_init, kernel_size=(3,3), strides=(2, 2), padding='same')(input_layer)
	new_depth = _d_init
	'''
	Output Sizes:
	*************
	...
	************
	...
	for dense_block_idx in range(_n_dense_blocks):
		...
		for dense_layer_idx in range(_n_dense_layers_per_block[dense_block_idx]):
			intermed = _dense_layer(intermed, new_depth, growth_rate,
									_growth_limit)
			new_depth += growth_rate
		if dense_block_idx != (_n_dense_blocks-1): # No Dense Transition for last layer
	...
	# flattened = Flatten()(intermed)

	gap_out = GlobalAveragePooling2D()(intermed)
	# gap_out = Dense(4096, activation='relu')(gap_out)
	# gap_out = Dense(2048, activation='relu')(gap_out)
	# gap_out = Dense(1024, activation='relu')(gap_out)
	
	# gap_out = Dense(512, activation='relu')(gap_out)
	gap_out = Dense(128, activation='relu')(gap_out)

	final_out = Dense(_n_classes, activation='softmax')(gap_out)
	model = Model(inputs=input_layer, outputs=final_out, name='Test_Net')
	return model
